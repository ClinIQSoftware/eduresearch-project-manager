...
def get_summary(db: Session = Depends(get_tenant_db)):
    ...
    today_time = (
        db.query(func.sum(TimeEntry.duration))
        .filter(func.date(TimeEntry.start_time) == today)
        .scalar()
        or 0
    )
...
def get_tasks_completed(days: int = 7, db: Session = Depends(get_tenant_db)):
    ...
    results = (
        db.query(
            func.date(Task.updated_at).label("date"), func.count(Task.id).label("count")
        )
        .filter(Task.status == "completed", Task.updated_at >= start_date)
        .group_by(func.date(Task.updated_at))
        .order_by(func.date(Task.updated_at))
        .all()
    )

    return [{"date": str(r.date), "completed": r.count} for r in results]
